---
title: What Cardano Is
subtitle: A stance, stated plainly — what I hold this chain to be, what that commits me to, and what would change my mind
type: stance
layer: 0
status: v1.0 (published position; re-versions with the corpus)
audience: [delegators, proposers, DReps, Constitutional Committee, anyone deciding whether to argue with me]
role: public anchor for the voting framework; the referent for "what the derivation cares about"
edges:
  - derives_from → money_ontology_paper (the unit as coordination index; the reification error)
  - derives_from → blockchain_money_primer (why a holder is not investor, user, or voter)
  - derives_from → holder_rights_articulation (the six rights)
  - refines → field-fitness-audit (why growth metrics are the wrong instrument)
  - tested_by → Proposal Intake and Sort (the three axes follow from §6)
  - tested_by → Abstention Spines
  - contested_by → the three asset readings (§8)
related_rights: [settlement access, unit of account integrity, informational integrity, governance participation, commons integrity, self-determination]
---
